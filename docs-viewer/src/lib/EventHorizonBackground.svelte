<script>
  ...
  const vertexShader = `
    ...
    void main() {
      ...
      float noiseFreq = 0.3;
      float noiseAmp = 2.0;
      float elevation = cnoise(vec3(pos.x * noiseFreq + uTime * 0.04, pos.y * noiseFreq, 0.0)) * noiseAmp;
      
      float dist = distance(uv, uHover);
      float gravity = (1.0 - smoothstep(0.0, 0.2, dist)) * 1.5 * uHoverState;
      elevation -= gravity;
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      float peaks = smoothstep(1.0, 3.0, vElevation);
  ...
  onMount(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uHoverState: { value: 0 },
        uColorStart: { value: new THREE.Color("#003344") },
        uColorEnd: { value: new THREE.Color("#00f5ff") }
      }
    });
